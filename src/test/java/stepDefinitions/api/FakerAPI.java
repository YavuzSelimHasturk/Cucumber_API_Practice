...
public class FakerAPI {
    ...
    @And("Faker apiden donen response'da status {string}, code {int}, total {int} oldugunu ve {string},{string},{string} ve {string} alanlarinin geldigini assert et")
    public void fakerApidenDonenResponseDaStatusCodeTotalOldugunuVeVeAlanlarininGeldiginiAssertEt(String status, int code, int total, String type, String number, String expiration, String owner) {

        CreditCardsPojo respPojo = response.as(CreditCardsPojo.class);
        assertEquals(status,respPojo.getStatus());
        assertEquals(code,respPojo.getCode());
        assertEquals(total,respPojo.getTotal());

        CreditCardsDataPojo respDataPojo = respPojo.getData()[0];
        assertFalse(respDataPojo.getType().isEmpty());
        assertFalse(respDataPojo.getNumber().isEmpty());
        assertFalse(respDataPojo.getExpiration().isEmpty());
        assertFalse(respDataPojo.getOwner().isEmpty());
