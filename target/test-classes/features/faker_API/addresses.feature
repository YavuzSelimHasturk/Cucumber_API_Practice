...
Feature: Faker API sorgu
  ...
  Scenario: Companies sorgusu

    Given Faker api URL icin gerekli path param "companies" ve query param "1"hazirla
    Then Faker apiden gelen cevabi response kaydet
    And Faker apiden donen response'da books status "OK", code 200 ve total 1 oldugunu ve de "country","website","zipcode","adress > county code" alanlarinin geldigini assert et
